---
title: TMP VistA Patch 863 DIBRG
doc_type: DIBR
...
![](tmp-vista-patch-863-dibrg/001.png)
...
|-----------|---------|------------------------|--------|
| 1/25/2024 | 1.0.0   | Initial Document Draft |        |
| 2/27/2024 | 1.0.1   | Changes for TV2        |        |
...
The Deployment, Installation, Back-Out and Rollback Guide defines the ordered, technical steps required to install the product, back out the installation (if necessary), and roll back to the previously installed version of the product. It provides installation instructions for the SD\*5.3\*863 patch

## Purpose 

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

The purpose of this Guide is to provide installation steps for the SD\*5.3\*863 patch. The intended audience for this document is the Information Resource Management Systems (IRMS) staff. Some of the main features of this patch are the following:

1.  Adds additional Institution and Station Number information to the Clinic Inquiry of the TeleHealth Inquiries.
2.  Displays the date the default provider was last updated in the Clinic Inquiry of the TeleHealth Inquiries.
3.  Adds the new Missing Institution Report to the Telehealth Management Toolbox.
4.  Fixes a hard error generated due to a missing variable when parsing the incoming HL7 messages and sends back the acknowledge message to TMP.
5.  Prevents VistA from sending No Appointment Availability transactions for days that should have availability.
6.  Corrects code that generated a null subscript error when referencing a Parent IEN in the SDEC APPT REQUEST file (#409.85).

## Dependencies 

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

Patch SD\*5.3\*859 must be installed prior to installing SD\*5.3\*863.
...
1.  Select the Verify Checksums in Transport Global option to confirm the integrity of the routines that are in the transport global. When prompted for the INSTALL NAME enter the patch or build name (ex. SD\*5.3\*863).
...
2.  At the Select INSTALL NAME prompt, enter your build SD\*5.3\*863.
...
No Post-Install for this patch.
...
For item 2, auditing of the DEFAULT PROVIDER field (#16) of the HOSPITAL LOCATION file (#44) is implemented. This must be manually reversed in FileMan.

For items 1, 3, 4 and 5 the Back-out Procedure can be verified by printing the first 2 lines of the routine contained in this patch using the option First Line Routine Print \[XU FIRST LINE PRINT\].
...
User Acceptance Testing will be completed by the time patch SD\*5.3\*863 is released.
...
Back-out will be done only with the concurrence and participation of the development team and appropriate VA site/region personnel. The decision to back-out or rollback software will be a joint decision between the development team, VA site/region personnel and other appropriate VA personnel.
...
This is not a routine-only patch. Auditing of the DEFAULT PROVIDER field (#16) of the HOSPITAL LOCATION file (#44) is implemented. This must be manually reversed in FileMan.
...
There is one item to address to roll back this patch.

For item 2, auditing of the DEFAULT PROVIDER field (#16) of the HOSPITAL LOCATION file (#44) is implemented. This must be manually reversed in FileMan.

There are no other data changes in this patch.
