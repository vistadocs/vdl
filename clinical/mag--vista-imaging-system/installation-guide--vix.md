...
Contents
[Appendix A: VIX Install Checklist 56](#appendix-a-vix-install-checklist)
[VIX Install Checklist 56](#vix-install-checklist)
...
<tr class="header">
<th><blockquote>
<p>![](mag-3-197-vix-installation-guide/002.png)</p>
</blockquote></th>
<th><blockquote>
...
- ![](mag-3-197-vix-installation-guide/004.png)Critical information is indicated by:
...
> In addition to this manual, the following document contains information about the VIX: <span class="mark">REDACTED</span>
...
- Acquiring Installation Files
  - MAG3_0P197_VIX_Setup.msi and SQLEXPRESS_X64-14_0_1000_169.ZIP found in the VIXSqlInstaller folder should be copied to a temporary folder on the desktop.
  - Copy the MAG3_0P185T8_VIX_Setup.msi (or P177) for the backup plan if it is not already on the same VIX server.
...
> ![](mag-3-197-vix-installation-guide/005.png)

> *Figure 1: STRONGLY RECOMMENDED VIX Settings*

> ![](mag-3-197-vix-installation-guide/006.png)
...
> ![](mag-3-197-vix-installation-guide/007.png)If you cannot this access this site, enter a ticket and indicate that your site ACL (access control list) needs to be modified to access <span class="mark">REDACTED</span>.
...
1.  <span id="_bookmark12" class="anchor"></span>Start \| Search regedit \| Open Registry Editor
...
> If it occurred, please check/verify the Access control of VIX service account \[apachetomcat\] for the following folders, must have the Modify & Write access right (Allow access must be checked) -
...
> By mouse Right-click on the Folder, select 'Properties' - 'Security' - 'Edit'

> Otherwise, the Tomcat8 or VIX service(s) may experience some start-up, or a slowness issue.
...
> <span id="_bookmark13" class="anchor"></span>Preparing Passwords, Activating Components, and Staging Files
...
![](mag-3-197-vix-installation-guide/008.png)![](mag-3-197-vix-installation-guide/009.png)

> ![](mag-3-197-vix-installation-guide/010.png)
...
7.  This step is not required, but it could speed up the P197 installation by stopping the following VIX services first:
    ...
    - . Listener Tool
    - . VIX Render Service, VIX Viewer Service
    - (For other OS version, please find Admin Tools – Local Services to stop VIX service)
...
2.  Copy the latest VIX installer (MAG3_0P197_VIX_Setup.msi) and VIXSqlInstaller folder SQLEXPRESS_X64-14_0_1000_169.ZIP ) to a temporary folder on the desktop.
...
> ![](mag-3-197-vix-installation-guide/012.png)
...
8.  When the Install the VIX Prerequisites page displays, verify that the icons for the first two items on the page are ![](mag-3-197-vix-installation-guide/013.png) .
...
9.  On the same page, check the line that indicates the state of the Java Runtime environment. If ![](mag-3-197-vix-installation-guide/014.png) is shown, do the following:
    1.  Click Install.
    2.  Wait until the status icon the Java Runtime Environment changes to ![](mag-3-197-vix-installation-guide/015.png) . (This install of Java will take longer than previous versions and may take several minutes to complete)
10. On the same page, check the line that indicates the state of the Apache Tomcat installation. If ![](mag-3-197-vix-installation-guide/016.png) is shown, do the following:
    1.  Click Install.
    2.  In the dialog box that displays, enter and confirm the Apache Tomcat password that you prepared as described in [<u>Preparing Passwords, Activating Components,</u> <u>and Staging Files</u>](#_bookmark13) previously. Then, click OK.
    3.  Wait until the status icon for Apache Tomcat changes to ![](mag-3-197-vix-installation-guide/017.png) . (This install of Tomcat will take longer than previous versions and may take several minutes to complete)
11. On the same page, check the line that indicates the state of the Laurel Bridge toolkit. If ![](mag-3-197-vix-installation-guide/018.png) is shown, do the following:
...
12. On the same page, check the line that indicates the state of the service account. If ![](mag-3-197-vix-installation-guide/019.png) is shown, do the following:
    1.  Click Create.
    2.  In the dialog box that displays, enter the Windows service account password that you prepared [<u>Preparing Passwords, Activating Components, and Staging Files</u>](#_bookmark13) previously. Then, click OK.
    3.  Wait until the status icon for the service account changes to ![](mag-3-197-vix-installation-guide/020.png) .
13. On the same page, check the line that indicates the state of VIX security certificate. If ![](mag-3-197-vix-installation-guide/021.png) is shown, do the following:
    1.  Click Install.
...
14. Check the line that indicates VIX/Viewer Render Services. If ![](mag-3-197-vix-installation-guide/022.png) is shown, do the following.
    1.  Click Install
    2.  Click Configure Viewer/Render

![](mag-3-197-vix-installation-guide/023.png)![](mag-3-197-vix-installation-guide/024.png)
...
![](mag-3-197-vix-installation-guide/026.png)

15. In the Install the VIX Prerequisites page, confirm that all the icons are ![](mag-3-197-vix-installation-guide/027.png) . Then, click Next.
...
> ![](mag-3-197-vix-installation-guide/030.png)
...
![](mag-3-197-vix-installation-guide/034.png)

3.  ![](mag-3-197-vix-installation-guide/035.png)Verify VIX viewer and VIX render services are running.
4.  Verify the listener tool is running.

![](mag-3-197-vix-installation-guide/036.png)
...
![](mag-3-197-vix-installation-guide/037.png)

> *Figure 3: STRONGLY RECOMMENDED VIX Settings*

> ![](mag-3-197-vix-installation-guide/038.png)

> *Figure 4: MINIMUM VIX Settings*

> <span id="_bookmark18" class="anchor"></span>VistA Software Dependencies
...
> <span id="_bookmark19" class="anchor"></span>Scheduling Downtime and Impact of a VIX Update
...
> <span id="_bookmark20" class="anchor"></span>Java Version
...
1.  Uninstall all or any old MS-SQL programs (Control Panel – Program – Program and Feature – Uninstall)
2.  Remove prior Tomcat software other than Tomcat 8.0 (I.e.: 6.0)
3.  Remove/Delete the folder C:\Program Files\Microsoft SQL Server (include data files if it exists)

> Note: User may be prompted to restart the server during or after uninstalling SQL software. If prompted, accept the restart.

> For example:

> ![](mag-3-197-vix-installation-guide/039.png)

> Stop VIX Services (Apache Tomcat, VIX Viewer, and VIXRender)

> This step is optional but highly recommended to speed up installation process:
...
1.  Use an administrator-level account to log on to the standalone server where the VIX will be installed.
2.  Copy the VIX installation files (MAG3_0P197_VIX_Setup.msi and SQLEXPRESS_X64-14_0_1000_169.ZIP ) found in the VIXSqlInstaller folder to a local folder on the server.
...
![](mag-3-197-vix-installation-guide/041.png)
...
![](mag-3-197-vix-installation-guide/042.png)

> *Figure 5: Updating from 185 to 197*
...
- Check the line that indicates the state of the Java Runtime environment. If ![](mag-3-197-vix-installation-guide/043.png) is shown, do the following:
  - Click Install.
  - Wait until the status icon the Java Runtime Environment changes to ![](mag-3-197-vix-installation-guide/044.png) . (This install of Java will take longer than previous versions and may take several minutes to complete)
- Check the line that indicates the state of the Apache Tomcat installation. If ![](mag-3-197-vix-installation-guide/045.png) is shown, do the following:
  - Click Install.
  - In the dialog box that displays, enter and confirm the Apache Tomcat password that you prepared as described in [<u>Preparing Passwords,</u> <u>Activating Components, and Staging Files</u>](#_bookmark13) previously. Then, click OK.
  - Wait until the status icon for Apache Tomcat changes to ![](mag-3-197-vix-installation-guide/046.png) . (This install of Tomcat will take longer than previous versions and may take several minutes to complete)
12. On the same page, check the line that indicates the state of the service account. If ![](mag-3-197-vix-installation-guide/047.png) is shown, do the following:

> ![](mag-3-197-vix-installation-guide/048.png)o Click Assign.

- If your service user did not change, click Yes. If you need to reassign the password, click No.

![](mag-3-197-vix-installation-guide/049.png)

- If you answered No, in the dialog box that displays, enter the Windows service account password that you prepared [<u>Preparing Passwords,</u> <u>Activating Components, and Staging Files</u>](#_bookmark13) previously. Then, click OK.
- Wait until the status icon for the service account changes to ![](mag-3-197-vix-installation-guide/050.png) .
13. Check the line that indicates VIX/Viewer Render Services.
    1.  If ![](mag-3-197-vix-installation-guide/051.png) is shown, click Install. If ![](mag-3-197-vix-installation-guide/052.png) is shown, click Reinstall.
    2.  Click Configure Viewer/Render

> ![](mag-3-197-vix-installation-guide/053.png)![](mag-3-197-vix-installation-guide/054.png)
...
16. In the Specify the Release of Information (ROI) Configuration, do the following:
- Specify the access and verify codes for the account with the ROI periodic processing credentials. The VIX uses this account for periodic processing of ROI disclosure requests. The account must be valid VistA credentials with the MAG DICOM VISA secondary menu option and the OR CPRS GUI CHART secondary menu option. The credentials can be the credentials of the same service account that the DICOM Gateway and the HDIG use.
- Specify the email address that gets notifications for invalid ROI periodic processing credentials. The VIX sends an email notification to the address or addresses specified in this field if the ROI periodic processing credentials are expired or invalid. You can enter several addresses, separated by a comma.

> ![](mag-3-197-vix-installation-guide/059.png)
...
18. ![](mag-3-197-vix-installation-guide/061.png)On the Install the VIX page, click Install. (The information in this page is saved in C:\Program Files (x86)\VistA\Imaging\VixInstaller for future reference or troubleshooting.) This will start the installation process. It will also start the Tomcat and Viewer/Render services.
19. The VIX will start automatically when installation is complete.
20. Click Finish. The VIX Installation Wizard will close.

![](mag-3-197-vix-installation-guide/062.png)

21. Continue to [*<u>Verifying VIX Operations</u>*](#verifying-vix-operations)
...
![](mag-3-197-vix-installation-guide/063.png)

![](mag-3-197-vix-installation-guide/064.png)
...
> <span id="_bookmark25" class="anchor"></span>Verifying Access to the VIX Transaction Log
...
> <span id="_bookmark26" class="anchor"></span>Spot-checking VIX Image Delivery
...
![](mag-3-197-vix-installation-guide/065.png)

7.  Verify VIX Viewer and VIX Render services are running.

![](mag-3-197-vix-installation-guide/066.png)

8.  Verify listener tool is running.

![](mag-3-197-vix-installation-guide/067.png)

9.  If this is an installation of the VIX, please go back to [<u>Post-Installation Steps</u>](#post-installation).
...
> <span id="_bookmark28" class="anchor"></span>Reconfiguring a VIX – Standalone Server

1.  Review the information in the [*Scheduling Downtime and Impact of a VIX Update*](#_bookmark19)
...
![](mag-3-197-vix-installation-guide/068.png)
...
> <span id="_bookmark31" class="anchor"></span>Resuming Installation (single server VIX)
...
![](mag-3-197-vix-installation-guide/069.png)
...
> <span id="_bookmark35" class="anchor"></span>Uninstall/Restore as part of Troubleshooting
...
> <span id="_bookmark36" class="anchor"></span>Relocating a VIX onto a New Server
...
4.  Re-execute the VIX installation as if for a new standalone VIX, see [*<u>New VIX</u><u>Installation – Standalone Server</u>*](#_bookmark12) section. .

> <span id="_bookmark37" class="anchor"></span>Decommissioning a VIX
...
> ![](mag-3-197-vix-installation-guide/070.png)These steps will completely remove the VIX and permanently delete the VIX cache.
...
> <span id="_bookmark39" class="anchor"></span>Stopping the VIX service

> The steps for stopping the VIX service vary based on operating system.

> <span id="_bookmark40" class="anchor"></span>Remove VIX-related applications, accounts, directories, and variables
...
> ![](mag-3-197-vix-installation-guide/071.png)SKIP THIS STEP if you are uninstalling and reinstalling the VIX on the same server for troubleshooting purposes. If you delete these folders, you will need to recreate the VIX configuration and request new VIX security certificates.
...
- Remove these three programs (no reboot required): Apache Tomcat 8.0.45

> Java (TM) 1.8.0. Update 121 VIX Service Installation Wizard
...
<tr class="odd">
<td>☐</td>
<td>Install patch 197 KIDS in VistA</td>
<td>See Patch Description Document</td>
</tr>
<tr class="even">
<td>☐</td>
<td>Verify VIX server/VM meets minimum hardware specifications</td>
<td>Allocate more CPU cores/RAM as needed to meet minimum or recommended specifications</td>
</tr>
<tr class="odd">
<td>☐</td>
<td>Verify Microsoft Visual C++ 2010 Redistributable Package (x64) is installed</td>
<td>Install Microsoft Visual C++ 2010 Redistributable Package (x64) if required. (Download from SFTP site)</td>
</tr>
<tr class="even">
<td>☐</td>
<td>Verify .NET 3.5 is installed and enabled</td>
<td>Install and enable .NET 3.5 if required</td>
</tr>
<tr class="odd">
<td>☐</td>
<td>Verify .NET 4.5 is installed and enabled</td>
<td>Install and enable .NET 4.5 if required (Download from SFTP site)</td>
</tr>
<tr class="even">
<td>☐</td>
<td>Verify you have the VIXSqlInstaller folder</td>
<td>Download from SFTP site if required</td>
</tr>
<tr class="odd">
<td>☐</td>
<td>Initiate the installation of the 197 VIX</td>
<td>See Installation Guide</td>
</tr>
<tr class="even">
<td>☐</td>
...
<td>Verify in window services console</td>
</tr>
<tr class="odd">
<td>☐</td>
<td>Verify ListenerTool service is running</td>
<td>Verify in window services console</td>
</tr>
<tr class="even">
<td>☐</td>
<td>Verify that MacAfee Exclusions are Updated</td>
<td>See Installation Guide</td>
</tr>
<tr class="odd">
<td>☐</td>
<td>Request Site Service update to your facility if needed</td>
<td><p>Enter in national CA ticket requesting CLIN3 to update your site service entry to</p>
<p>include the new viewer. (Port 343)</p></td>
</tr>
<tr class="even">
<td>☐</td>
<td>Verify that images can be viewed via zero-footprint Image Viewer.</td>
<td><p>Verify JLV or other application utilizing zero footprint viewer. If unable to view an</p>
<p>image, enter a support ticket for assistance.</p></td>
</tr>
</tbody>
</table>
...
### From: MAG*3*185 VIX Installation Guide

## ![](mag-3-185-vix-installation-guide/003.png)Document Conventions
...
- Sample output is shown in monospace.
- Important or required information is shown in a Note.
- ![](mag-3-185-vix-installation-guide/004.png)Critical information is indicated by:
