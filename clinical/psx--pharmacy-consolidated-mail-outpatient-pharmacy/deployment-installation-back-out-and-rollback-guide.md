---
title: PSX*2*97 Deployment, Installation, Back-out, and Rollback Guide
doc_type: DIBR
...
- XUPROGMODE
menu_options: 0
description: '| Date | Version | Description | Author | |---------------|-------------|-----------------|----------------------| | December 2022 | 1.0 | Initial Version | Liberty IT Solutions'
...
docx_url: https://www.va.gov/vdl/documents/Clinical/Pharm-Consol_Mail_Outpat_Pharm_(CMOP)/psx_2_0_p97_dibrg.docx
pdf_url: https://www.va.gov/vdl/documents/Clinical/Pharm-Consol_Mail_Outpat_Pharm_(CMOP)/psx_2_0_p97_dibrg.pdf
app_url: https://www.va.gov/vdl/application.asp?appid=85
audit_applied: '2026-05-31'
master_source: PSX*2*97 Deployment, Installation, Back-out, and Rollback Guide
master_pub_date: December 2022
consolidated_from: 2 versions
prior_versions:
- PSX*2*98 Deployment, Installation, Back-out, and Rollback Guide
...
![](psx-2-97-deployment-installation-back-out-and-rollback-guide/001.png)
...
|---------------|-------------|-----------------|----------------------|
| December 2022 | 1.0         | Initial Version | Liberty IT Solutions |
...
This document describes how to deploy and install the Outpatient Pharmacy (OP) Suspended Rx Parameters patch PSX\*2.0\*97, as well as how to back-out the product and rollback to a previous version or data set. This document is a companion to the project charter and management plan for this effort.

## Purpose

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

The purpose of this plan is to provide a single, common document that describes how, when, where, and to whom the OP Suspended Rx Parameters project will be deployed and installed, as well as how it is to be backed out and rolled back, if necessary. The plan also identifies resources, communications plan, and rollout schedule.

## Dependencies

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

PSX\*2.0\*90 must be installed BEFORE PSX\*2.0\*97

PSO\*7.0\*694 must be installed BEFORE PSX\*2.0\*97
...
| 6      | Office of Information and Technology (OIT), Development, Security, and Operations (DevSecOps) Infrastructure Operations (IO) Vista Applications Division and Individual Veterans Administration Medical Centers (VAMCs) | Installation       | Plan and schedule installation                                                                                      | Deployment                       |
...
The deployment and installation is scheduled to run for 31 days, as depicted in the master deployment schedule for Pharmacy Operational Updates.

| Task              | Start    | Finish   |
|-------------------|----------|----------|
| National Release  | 12/07/22 | 12/07/22 |
| Compliance Period | 12/08/22 | 1/07/23  |

## Site Readiness Assessment 

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

This section discusses the locations that will receive the deployment of the OP Suspended Rx Parameters patch PSX\*2.0\*97.
...
Below is a list of the applications involved in this project along with their patch numbers. The patches are listed in the order of installation.

APPLICATION/VERSION PATCH

OUTPATIENT PHARMACY v7.0 PSO\*7.0\*694

CMOP v2.0 PSX\*2.0\*97
...
The installation of this patch can be queued but should not be queued at a time when the CMOP transmission processes are scheduled or are in progress. There are two scheduled jobs which should be checked:

Scheduled CS Transmission \[PSXR SCHEDULED CS TRANS\] and Scheduled Non-CS Transmission \[PSXRSCHEDULED NON-CS TRANS\].

The patches MUST BE INSTALLED in the following order:
...
2.  From the Kernel Installation and Distribution System Menu, select the Installation Menu. From this menu,
    1.  Select the Verify Checksums in Transport Global option to confirm the integrity of the routines that are in the transport global. When prompted for the INSTALL NAME enter the patch name, PSX\*2.0\*97.
    2.  Select the Backup a Transport Global option to create a backup message. You must use this option and specify what to backup: the entire Build or just Routines. The backup message can be used to restore the routines and components of the build to the pre-patch condition.
        ...
        2.  At the Select INSTALL NAME prompt, enter your build PSX\*2.0\*97.
...
4.  Select the Install Package(s) option and choose the patch to install.
    ...
    4.  If prompted 'Delay Install (Minutes): (0 - 60): 0//', answer 0.
...
Back-out risks include the current state VistA handling of CMOP transmissions.
