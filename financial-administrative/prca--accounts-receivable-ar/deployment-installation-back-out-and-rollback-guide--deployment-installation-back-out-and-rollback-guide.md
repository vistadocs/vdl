---
title: PRCA*4.5*459 Deployment Installation, Back-out, and Rollback Guide
doc_type: DIBR
...
revision_count: 1
revision_newest: 10/20/2025
revision_oldest: 10/20/2025
docx_url: https://www.va.gov/vdl/documents/Financial_Admin/Accounts_Receivable_(AR)/prca_4_5_459_dibrg.docx
pdf_url: https://www.va.gov/vdl/documents/Financial_Admin/Accounts_Receivable_(AR)/prca_4_5_459_dibrg.pdf
app_url: https://www.va.gov/vdl/application.asp?appid=29
audit_applied: '2026-05-31'
master_source: PRCA*4.5*459 Deployment Installation, Back-out, and Rollback Guide
master_pub_date: October 2025
consolidated_from: 10 versions
prior_versions:
- PRCA*4.5*412 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*422 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*424 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*429 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*433 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*436 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*438 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*441 Deployment Installation, Back-out, and Rollback Guide
- PRCA*4.5*448 Deployment Installation, Back-out, and Rollback Guide
...
![](prca-4-5-459-deployment-installation-back-out-and-rollback-guide/001.png)
...
|------------|---------|------------------|-----------------------------------|
| 10/20/2025 | V1      | Initial release. | VHA Finance IBAR Development Team |
...
This document describes how to deploy and install the VHA Finance Accounts Receivable (AR) Enhancements patch PRCA\*4.5\*459 as well as how to back-out the product and rollback to a previous version or data set. This document is a companion to the project charter and management plan for this effort.

## Purpose

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

The purpose of this plan is to provide a single, common document that describes how, when, where, and to whom the VHA Finance Accounts Receivable Enhancements patch PRCA\*4.5\*459 will be deployed and installed, as well as how it is to be backed out and rolled back, if necessary. The plan also identifies resources, communications plan, and rollout schedule. Specific instructions for installation, back-out, and rollback are included in this document.

## Dependencies

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

The following patches must be installed prior to installing PRCA\*4.5\*459:

- PRCA\*4.5\*448 must be installed BEFORE PRCA\*4.5\*459.
...
|-----------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------|---------------------------------------------------------------------------------------------------------------------|
| VHA Finance Financial Management VistA Support Team                                                                                                       | Deployment      | Plan and schedule deployment (including orchestration with vendors)                                                 |
| VHA Finance Financial Management VistA Support team and existing local VA Medical Center (VAMC) and Consolidated Patient Account Center (CPAC) processes. | Deployment      | Determine and document the roles and responsibilities of those involved in the deployment.                          |
| VHA Finance Financial Management VistA Support team and Veteran-focused Integrated Process (VIP) Release Agent                                            | Deployment      | Test for operational readiness                                                                                      |
| VHA Finance Financial Management VistA Support team                                                                                                       | Deployment      | Execute deployment                                                                                                  |
| Designated VistA patch installer for this package                                                                                                         | Installation    | Plan and schedule installation                                                                                      |
| VistA office                                                                                                                                              | Installation    | Ensure proper change management approval obtained in support of national release                                    |
| CPAC Revenue Analysts                                                                                                                                     | Installations   | Coordinate training                                                                                                 |
| Designated VistA patch installer for this package, and CPAC Revenue Analysts, VHA Finance Financial Management VistA Support team, and Development Team   | Back-out        | Confirm availability of back-out instructions and back-out strategy (what are the criteria that trigger a back-out) |
...
The deployment and installation are scheduled to run for 30 days starting with the National Release date and concluding with the National Compliance date by which time all 130 VistA production instances should have the patch installed.

## Site Readiness Assessment

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

This section discusses the locations that will receive the VHA Finance Accounts Receivable patch PRCA\*4.5\*459 deployment.
...
All 130 VistA production instances. The Initial Operating Capabilities (IOC) test sites for this project were Edward J Hines VA Hospital (Chicago, IL) (578), VA Central Plains Health Network (Omaha Division) (636), and VA Western New York Health Care System (Buffalo) (528).
...
The VHA Finance Accounts Receivable Enhancements patch PRCA\*4.5\*459 is a VistA patch and does not require any special or specific resources other than an existing and functional VistA system.
...
1.  Select the Verify Checksums in Transport Global option to confirm the integrity of the routines that are in the transport global. When prompted for the INSTALL NAME enter the patch PRCA\*4.5\*459.
2.  Select the Backup a Transport Global option to create a backup message. You must use this option and specify what to back up, either the entire Build or just Routines.

    The backup message can be used to restore the routines and components of the build to the pre-patch condition.
    1.  At the Installation option menu, select Backup a Transport Global.
    2.  At the Select INSTALL NAME prompt, enter your build PRCA\*4.5\*459.
...
4.  Select the Install Package(s) option and choose the patch to install.
    1.  If prompted "Want KIDS to Rebuild Menu Trees Upon Completion of Install? YES//",

        If the environment you are installing in automatically rebuilds menus nightly, answer NO. Otherwise, answer YES.
...
The decision to back-out this VistA patch will be made by VHA Finance Financial Management VistA Support team, CPAC Revenue System Management staff, and the Development Team. Criteria to be determined based on separate and unique factors and will be evaluated upon post-patch installation use of the product.
...
The order would come from the Release Coordinator, Product Line Manager, and VHA Finance Financial Management VistA Support team. This should be done in consultation with the Development team.

## Back-Out Procedure

<!-- back-to-toc -->[↑ Table of Contents](#table-of-contents)

The back-out procedure for VistA applications is complex and not a "one size fits all" solution. The general strategy for a VistA back-out is to repair the code with a follow-up patch. The development team recommends that sites log a ticket if it is a nationally released patch. If not, the site should contact the VHA Finance Financial Management VistA Support team directly for specific solutions to their unique problems.

The PRCA\*4.5\*459 patch contains the following build components:

- Not Applicable (No build components for this patch)
...
Please contact the VHA Finance Financial Management VistA Support team for assistance since this installed patch contains components in addition to routines.
...
### From: PRCA*4.5*424 Deployment Installation, Back-out, and Rollback Guide
...
<span id="_Toc163809069" class="anchor"></span>Table : Hardware Specifications
